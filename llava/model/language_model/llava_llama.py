...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def __init__(self, config: LlamaConfig):
        ...
        self.xattn_layers = nn.ModuleList(
            [GatedCrossAttentionBlock(dim=4096, dim_visual=64, layer_idx=layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
    ...
    def forward(
        ...
        for i in range(self.num_hidden_layers):
            ...
            xattn_layer = self.xattn_layers[i]

            if output_hidden_states:
                all_hidden_states += (hidden_states,)

            # xattn layers to fuse image features
            hidden_states = xattn_layer(hidden_states, media)
